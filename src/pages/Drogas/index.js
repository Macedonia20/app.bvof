...
class Drogas extends Component {
  ...
  async buscarDados() {
    ...
    const { data } = await api.get(`/drogas`, options);
  ...
  async cadastrarItem() {
    try {
      ...
      const options = this.getOptions();
      await api.post('/drogas', novoItem, options)
      .then(() => {
        this.controlarModal();
  ...
  async editarItem() {
    try {
      ...
      const options = this.getOptions();
      await api.put(`/drogas/${idItem}`, itemSelecionado, options)
      .then(() => {
        this.controlarModal();
  ...
  async apagarPermanentemente() {
    ...
    const options = this.getOptions();
    await api.delete(`/drogas/${idItem}`, options)
    .then(()=> {
  ...
  render() {
    ...
    const columns = [{
      ...
      formatter: (cellContent, row) => (
        <React.Fragment>
          <Link to="#" onClick={() => this.controlarModalEdicao(row.iddrogas)} className="me-3 text-primary"><i className="mdi mdi-pencil font-size-18"></i></Link>
          <Link to="#" onClick={() => this.apagarItem(row.iddrogas)} className="text-danger"><i className="mdi mdi-trash-can font-size-18"></i></Link>
        </React.Fragment>
      ),
    }];
      ...
      <React.Fragment>
        ...
        <div className="page-content">
          <div className="container-fluid">
            <Breadcrumbs title="Drogas" breadcrumbItems={this.state.breadcrumbItems} />
